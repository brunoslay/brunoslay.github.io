const mesa = {
	...
	damage: function(p_damaged, char_damaged, p_atks, char_atks, conflito){
		...
		if (conflito.result === 2 && conflito.dodge === false) {
			...
			this.audio(conflito);
			setTimeout(_=>{this.audio(conflito);}, 150);
	...
	drawChars: function(char_name, sheetNumber){


		let img1 = document.getElementById("image1");
		let style1 = this.player1.characters[char_name]["sheet"+sheetNumber];
		let { height, width, img, divPos } = style1;

		img1.style = "background-color: green; height: "+height+"px; width: "+width+"px; background: url('"+img+"') 0px 0px; -webkit-transform: scaleX(-1); transform: scaleX(-1); position: absolute; right: "+divPos+"%; "
		...
		img2.style = "background-color: green; height: "+height+"px; width: "+width+"px; background: url('"+img+"') 0px 0px; position: absolute; left: "+divPos+"%; "
